...
interface ParticleTextEffectProps {
  words?: string[]
  frameDuration?: number // Duration each word displays (in frames)
}
...
const DEFAULT_WORDS = ["REWISE", "REMEMBER", "REVISE"]

export function ParticleTextEffect({ words = DEFAULT_WORDS, frameDuration = 90 }: ParticleTextEffectProps) {
  ...
  const nextWord = (word: string, canvas: HTMLCanvasElement) => {
    ...
    // Select color from brand palette
  ...
  const animate = () => {
    ...
    ctx.fillStyle = "rgba(10, 11, 13, 0.1)" // #0a0b0d (bg-dark) with alpha
    ...
    if (frameCountRef.current % frameDuration === 0) {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-bg-dark p-4">
      <canvas
        ref={canvasRef}
        className="rounded-lg shadow-2xl max-w-full h-auto cursor-pointer"
